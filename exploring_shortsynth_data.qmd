---
title: "Exploring the Synthetic Short-Timeframe Transaction Dataset"
...
rawdata_tbl <- read_rds("data/rawsynthdata_shortframe_transactions_tbl.rds")
...
data_tbl <- data_tbl |>
  mutate(
    tnx_dow   = format(tnx_timestamp, "%a") |> fct_relevel(dow_order),
    tnx_month = format(tnx_timestamp, "%b") |> fct_relevel(month_order),
    tnx_week  = format(tnx_timestamp, "%W") |> as.factor(),
...
customer_transactions_raw_tbl <- data_tbl

customer_transactions_raw_tbl |> glimpse()
```
...
btyd_data_lst <- customer_transactions_raw_tbl |>
  construct_btyd_data(
    last_date = use_fit_end_date
    )


btyd_transactions_tbl <- btyd_data_lst$tnxdata
btyd_transactions_tbl |> glimpse()

btyd_summarystats_tbl <- btyd_data_lst$cbsdata
btyd_summarystats_tbl |> glimpse()
```
...
shuffle_tbl <- btyd_summarystats_tbl |>
...
customer_fit_stats_tbl <- btyd_summarystats_tbl
customer_fit_stats_tbl |> glimpse()


customer_valid_stats_tbl <- customer_transactions_raw_tbl |>
  semi_join(customer_fit_stats_tbl, by = "customer_id") |>
  filter(
...
Finally, we want to ensure there is a proper lookup table for the original
(non-factor) `customer_id` and the one that is a factor.

```{r construct_customer_id_lookup}
#| echo: TRUE

customer_id_lookup_tbl <- btyd_summarystats_tbl |>
  transmute(
    customer_id,
    orig_id = customer_id |> as.character()
    ) |>
  distinct()
...
id_1000  |> write_rds("data/shortsynth_customer_subset_ids.rds")

btyd_summarystats_tbl |> write_rds("data/shortsynth_btyd_summarystats_tbl.rds")



customer_cohort_data_tbl |>
  write_rds("data/shortsynth_customer_cohort_data_tbl.rds")

customer_fit_stats_tbl |>
  write_rds("data/shortsynth_customer_fit_stats_tbl.rds")

customer_valid_stats_tbl |>
  write_rds("data/shortsynth_customer_valid_stats_tbl.rds")

customer_transactions_raw_tbl |>
  write_rds("data/shortsynth_transaction_data_tbl.rds")

obs_fitdata_tbl   |> write_rds("data/shortsynth_obs_fitdata_tbl.rds")
obs_validdata_tbl |> write_rds("data/shortsynth_obs_validdata_tbl.rds")
...
list(
  ...
  write_rds("data/shortsynth_simulation_dates.rds")

customer_id_lookup_tbl |>
  write_rds("data/shortsynth_customer_id_lookup_tbl.rds")
```
