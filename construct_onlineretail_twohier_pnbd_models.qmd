...
pnbd_onlineretail_twohiermean_stanfit$summary()
```
...
pnbd_onlineretail_twohierlambda_stanfit$summary()
```
...
model_assess_tbl <- dir_ls("data", regexp = "pnbd_onlineretail_(one|two|fixed).*_assess_.*simstats") |>
...
ggplot(model_assess_summstat_tbl) +
  ...
  theme(
    axis.text.x = element_text(angle = 20, vjust = 0.5, size = 6)
    )
```
