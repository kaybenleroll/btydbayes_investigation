...
```{r display_pnbd_init_short_fixed_model_stancode}
#| echo: FALSE
...
stan_modelname <- "pnbd_init_short_fixed"
...
if(!file_exists(stanfit_object_file)) {
  pnbd_init_short_fixed_stanfit <- pnbd_fixed_stanmodel$sample(
    data            =                stan_data_lst,
    chains          =                            4,
    iter_warmup     =                          500,
    iter_sampling   =                          500,
  ...
  pnbd_init_short_fixed_stanfit$save_object(stanfit_object_file)

} else {
  pnbd_init_short_fixed_stanfit <- read_rds(stanfit_object_file)
}

pnbd_init_short_fixed_stanfit$summary()
```


We have some basic HMC-based validity statistics we can check.

```{r calculate_pnbd_init_short_fixed_hmc_diagnostics}
#| echo: TRUE

pnbd_init_short_fixed_stanfit$cmdstan_diagnose()
```
...
pnbd_init_short_fixed_stanfit$draws(inc_warmup = TRUE) |>
  mcmc_trace(
...
pnbd_init_short_fixed_stanfit$draws(inc_warmup = FALSE) |>
...
```{r plot_pnbd_init_short_fixed_parameter_rhat}
#| echo: TRUE

pnbd_init_short_fixed_stanfit |>
...
```{r plot_pnbd_init_short_fixed_parameter_neffratio}
#| echo: TRUE
...
```{r plot_pnbd_init_short_fixed_parameter_acf}
#| echo: TRUE

pnbd_init_short_fixed_stanfit$draws() |>
...
```{r construct_pnbd_init_short_fixed_validation_qvalues}
#| echo: TRUE

pnbd_init_short_fixed_valid_lst <- create_pnbd_posterior_validation_data(
  stanfit       = pnbd_init_short_fixed_stanfit,
  data_tbl      = customer_fit_stats_tbl,
  simparams_tbl = customer_simparams_tbl
  )

pnbd_init_short_fixed_valid_lst$lambda_qval_plot |> plot()

pnbd_init_short_fixed_valid_lst$mu_qval_plot |> plot()
```
...
pnbd_init_short_fixed_simstats_tbl <- construct_pnbd_posterior_statistics(
  stanfit         = pnbd_init_short_fixed_stanfit,
  fitdata_tbl     = customer_fit_stats_tbl
  )

pnbd_init_short_fixed_simstats_tbl |> glimpse()
```
...
```{r setup_simulation_pnbd_init_short_fixed_fitdata_transactions}
#| echo: TRUE

fit_label <- "pnbd_init_short_fixed"
...
pnbd_init_short_fixed_fitsims_index_tbl <- pnbd_init_short_fixed_simstats_tbl |>
  mutate(
...
pnbd_init_short_fixed_fitsims_index_tbl |> glimpse()
```

We now use this setup to generate our simulations.


```{r generate_simulation_pnbd_init_short_fixed_fitdata_transactions}
#| echo: TRUE
...
runsims_tbl <- pnbd_init_short_fixed_fitsims_index_tbl |>
  anti_join(precomputed_tbl, by = "sim_file")


if(nrow(runsims_tbl) > 0) {
  pnbd_init_short_fixed_fitsims_index_tbl <- runsims_tbl |>
    mutate(
...
```{r retrieve_pnbd_init_short_fixed_fit_simstats}
#| echo: TRUE
...
pnbd_init_short_fixed_fit_simstats_tbl <- pnbd_init_short_fixed_fitsims_index_tbl |>
  mutate(
    sim_data = future_map(
...
pnbd_init_short_fixed_fit_simstats_tbl |> glimpse()
```
...
```{r compare_pnbd_init_short_fixed_fit_multitransaction_customer_counts}
#| echo: TRUE
...
sim_data_tbl <- pnbd_init_short_fixed_fit_simstats_tbl |>
...
```{r compare_pnbd_init_short_fixed_fit_total_transaction_count}
#| echo: TRUE
...
```{r compare_pnbd_init_short_fixed_fit_transaction_quantiles}
#| echo: TRUE
...
```{r setup_simulation_pnbd_init_short_fixed_validdata_transactions}
#| echo: TRUE
...
pnbd_init_short_fixed_validsims_index_tbl <- pnbd_init_short_fixed_simstats_tbl |>
  mutate(
...
pnbd_init_short_fixed_validsims_index_tbl |> glimpse()
```
...
```{r generate_pnbd_init_short_fixed_validdata_transactions}
#| echo: TRUE
...
runsims_tbl <- pnbd_init_short_fixed_validsims_index_tbl |>
  anti_join(precomputed_tbl, by = "sim_file")


if(nrow(runsims_tbl) > 0) {
  pnbd_init_short_fixed_validsims_index_tbl <- runsims_tbl |>
    mutate(
      chunk_data = future_map2_int(
        cust_params, sim_file,
        run_simulations_chunk,
...
```{r retrieve_pnbd_init_short_fixed_valid_simstats}
#| echo: TRUE
...
pnbd_init_short_fixed_valid_simstats_tbl <- pnbd_init_short_fixed_validsims_index_tbl |>
  mutate(
    sim_data = future_map(
      sim_file, retrieve_sim_stats,
...
pnbd_init_short_fixed_valid_simstats_tbl |> glimpse()
```
...
```{r compare_pnbd_init_short_fixed_valid_multitransaction_customer_counts}
#| echo: TRUE
...
sim_data_tbl <- pnbd_init_short_fixed_valid_simstats_tbl |>
  filter(sim_tnx_count > 0) |>
  count(draw_id, name = "sim_customer_count")
...
```{r compare_pnbd_init_short_fixed_valid_total_transaction_count}
#| echo: TRUE
...
```{r compare_pnbd_init_short_fixed_valid_transaction_quantiles}
#| echo: TRUE
