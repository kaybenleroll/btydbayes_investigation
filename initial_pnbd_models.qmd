...
customer_simparams_tbl    <- read_rds("data/rawsynthdata_shortframe_simparams_tbl.rds")
...
customer_fit_tnxdata_tbl <- read_rds("data/shortsynth_customer_fit_tnxdata_tbl.rds")
customer_fit_tnxdata_tbl |> glimpse()

customer_fit_summarystats_tbl <- read_rds("data/shortsynth_customer_fit_summarystats_tbl.rds")
customer_fit_summarystats_tbl |> glimpse()

customer_valid_tnxdata_tbl <- read_rds("data/shortsynth_customer_valid_tnxdata_tbl.rds")
customer_valid_tnxdata_tbl |> glimpse()

customer_valid_summarystats_tbl <- read_rds("data/shortsynth_customer_valid_summarystats_tbl.rds")
customer_valid_summarystats_tbl |> glimpse()
```
...
customer_fit_subset_summarystats_tbl <- customer_fit_summarystats_tbl |>
  filter(customer_id %in% customer_subset_id)

customer_fit_subset_summarystats_tbl |> glimpse()

customer_fit_subset_tnxdata_tbl <- customer_fit_tnxdata_tbl |>
  filter(customer_id %in% customer_subset_id)

customer_fit_subset_tnxdata_tbl |> glimpse()


customer_valid_subset_summarystats_tbl <- customer_valid_summarystats_tbl |>
  filter(customer_id %in% customer_subset_id)

customer_valid_subset_summarystats_tbl |> glimpse()

customer_valid_subset_tnxdata_tbl <- customer_valid_tnxdata_tbl |>
  filter(customer_id %in% customer_subset_id)

customer_valid_subset_tnxdata_tbl |> glimpse()
...
customer_fitdata_transactions_tbl <- customer_transactions_tbl |>
  ...
  select(
    customer_id, tnx_timestamp, tnx_dow, tnx_month, tnx_week, tnx_amount
    )
...
stan_data_lst <- customer_fit_summarystats_tbl |>
  select(cust_id, x, t_x, T_cal) |>
  compose_data(
    mu_mn     = 0.10,
    mu_cv     = 1.00,
...
low_neff_ratio_vars <- pnbd_init_stanfit |>
  neff_ratio(pars = c("lambda", "mu")) |>
  sort() |>
  head(30) |>
  names()
...
pnbd_init_stanfit$draws() |>
  mcmc_acf(pars = low_neff_ratio_vars[1:6]) +
...
pnbd_init_valid_lst <- create_pnbd_posterior_validation_data(
  ...
  data_tbl      = customer_fit_summarystats_tbl,
...
pnbd_stanfit <- pnbd_init_stanfit |>
  recover_types(customer_fit_summarystats_tbl |> select(-customer_id))

pnbd_init_simstats_tbl <- construct_pnbd_posterior_statistics(
  stanfit         = pnbd_stanfit,
  fitdata_tbl     = customer_fit_subset_summarystats_tbl
  )
...
obs_customer_count <- customer_fit_summarystats_tbl |>
  filter(x > 0) |>
  nrow()
...
obs_total_count <- customer_fit_summarystats_tbl |>
  pull(x) |>
  sum()
...
obs_quantiles_tbl <- customer_fit_summarystats_tbl |>
  reframe(
...
obs_customer_count <- customer_valid_summarystats_tbl |>
  filter(tnx_count > 0) |>
  nrow()
...
obs_total_count <- customer_valid_summarystats_tbl |>
  pull(tnx_count) |>
  sum()
...
obs_quantiles_tbl <- customer_valid_summarystats_tbl |>
  reframe(
    prob_label = c("p10", "p25", "p50", "p75", "p90", "p99"),
...
stan_data_lst <- customer_fit_tnxdata_tbl |>
  transmute(cust_id, amt = tnx_amount) |>
  compose_data(
    custhier_mn = 200.00,
    custhier_cv =   0.50,
...
stan_data_lst$n_cust  <- stan_data_lst$n_cust_id
stan_data_lst$cust_id <- stan_data_lst$cust_id
...
Finally, we also want to look at autocorrelation in the chains for each
parameter. We look at the ACFs for the parameters with the lowest $N_{eff}$
ratios.

```{r plot_tnxamt_init_parameter_acf}
#| echo: TRUE

low_neff_vars <- tnxamt_init_stanfit |>
  neff_ratio(pars = c("amt_mn", "amt_alpha")) |>
  sort() |>
  head(20) |>
  names()
...
tnxamt_init_stanfit$draws() |>
  mcmc_acf(pars = low_neff_vars[1:8]) +
    ggtitle("Autocorrelation Plot of Sample Values")
```
...
As before, for our simulated data we have the 'correct' values for the
parameters and so we can calibrate our model against this.

```{r check_tnxamt_model_fit}
#| echo: TRUE

tnxamt_init_valid_tbl <- tnxamt_init_stanfit |>
  recover_types(customer_fit_tnxdata_tbl |> select(-customer_id, -invoice_id)) |>
  spread_draws(amt_mn[cust_id], amt_alpha[cust_id]) |>
  inner_join(
    customer_simparams_tbl |> select(customer_id, customer_amtmn),
    by = c("cust_id" = "customer_id")
    ) |>
  rename(customer_id = cust_id) |>
  calculate_distribution_qvals(amt_mn, customer_amtmn, customer_id)

ggplot(tnxamt_init_valid_tbl) +
  geom_histogram(aes(x = q_val), bins = 50) +
  geom_hline(
    aes(yintercept = tnxamt_init_valid_tbl |> nrow() |> divide_by(50)),
    colour = "red"
    ) +
  labs(
    x = "q-Values",
    ...
    title = "Histogram of the q-Values for Amount Mean"
    )

```

We are not running simulation validation yet as to do this properly we need
a combined model of both the transaction and amounts so we can get a better
assessment of our simulations.


# Fit Initial Combined Transaction Model

We now want to construct a combination of the P/NBD model and the Gamma/Gamma
model so we can both simulation transactions and the transaction amounts.

We build the Stan model by combining the two Stan models. This process is
straightforward.

```{r display_pnbd_init_combined_model_stancode}
#| echo: FALSE

read_lines("stan_code/pnbd_init_combined.stan") |> cat(sep = "\n")
```
...
```{r compile_pnbd_init_combined_stanmodel}
#| echo: TRUE
#| results: "hide"

pnbd_init_combined_stanmodel <- cmdstan_model(
  "stan_code/pnbd_init_combined.stan",
  include_paths =   stan_codedir,
  pedantic      =           TRUE,
  dir           =  stan_modeldir
  )
...
```{r fit_pnbd_init_combined_stanmodel}
#| echo: TRUE

stan_modelname <- "pnbd_init_combined"
stanfit_prefix <- str_c("fit_", stan_modelname)

stanfit_object_file <- glue("data/{stanfit_prefix}_stanfit.rds")


stan_data_summ_lst <- customer_fit_summarystats_tbl |>
  select(cust_id, x, t_x, T_cal) |>
  compose_data()

stan_data_lst <- customer_fit_tnxdata_tbl |>
  select(cust_id = customer_id, amt = tnx_amount) |>
  compose_data(
    lambda_mn   = 0.25,
    lambda_cv   = 1.00,
...
stan_data_lst$n_cust <- stan_data_summ_lst$n
stan_data_lst$x      <- stan_data_summ_lst$x
stan_data_lst$t_x    <- stan_data_summ_lst$t_x
stan_data_lst$T_cal  <- stan_data_summ_lst$T_cal


if(!file_exists(stanfit_object_file)) {
  pnbd_init_combined_stanfit <- pnbd_init_combined_stanmodel$sample(
    data            =                stan_data_lst,
    chains          =                            4,
    iter_warmup     =                          500,
    iter_sampling   =                          500,
  ...
  pnbd_init_combined_stanfit$save_object(stanfit_object_file, compress = "gzip")

} else {
  pnbd_init_combined_stanfit <- read_rds(stanfit_object_file)
}

pnbd_init_combined_stanfit$print()
```


We have some basic HMC-based validity statistics we can check.

```{r calculate_pnbd_init_combined_hmc_diagnostics}
#| echo: TRUE

pnbd_init_combined_stanfit$cmdstan_diagnose()
```
...
```{r plot_combined_model_traceplots}
#| echo: TRUE
...
pnbd_init_combined_stanfit$draws() |>
  mcmc_trace(pars = parameter_subset) +
  expand_limits(y = 0) +
  labs(
    x = "Iteration",
...
```{r plot_combined_model_parameter_rhat}
#| echo: TRUE

pnbd_init_combined_stanfit |>
  rhat(pars = c("lambda", "mu", "amt_mn", "amt_alpha")) |>
  mcmc_rhat() +
    ggtitle("Plot of Parameter R-hat Values")
```
...
```{r plot_combined_model_parameter_neffratio}
#| echo: TRUE

pnbd_init_combined_stanfit |>
  neff_ratio(pars = c("lambda", "mu", "amt_mn", "amt_alpha")) |>
  mcmc_neff() +
    ggtitle("Plot of Parameter Effective Sample Sizes")
```
...
```{r plot_combined_model_parameter_acf}
#| echo: TRUE

low_neff_vars <- pnbd_init_combined_stanfit |>
  neff_ratio(pars = c("lambda", "mu", "amt_mn", "amt_alpha")) |>
  sort() |>
  head(20) |>
  names()
...
```{r calculate_pnbd_init_combined_simstats}
#| echo: TRUE

pnbd_stanfit <- pnbd_init_combined_stanfit |>
  recover_types(customer_fit_summarystats_tbl |> select(-customer_id))

pnbd_init_combined_simstats_tbl <- construct_combined_model_posterior_statistics(
  stanfit         = pnbd_stanfit,
  fitdata_tbl     = customer_fit_subset_summarystats_tbl
  )

pnbd_init_combined_simstats_tbl |> glimpse()
```

We now want to write out the simulation stats to disk.

```{r write_fit_pnbd_init_combined_simstats}
#! echo: TRUE

pnbd_init_combined_simstats_tbl |>
  write_rds("data/pnbd_init_combined_assess_model_simstats_tbl.rds", compress = "gz")
```
...
```{r setup_simulation_pnbd_init_combined_fitdata_transactions}
#| echo: TRUE

fit_label <- "pnbd_init_combined"
...
pnbd_init_combined_fitsims_index_tbl <- pnbd_init_combined_simstats_tbl |>
  mutate(
    start_dttm = first_tnx_date   |> as.POSIXct(),
    end_dttm   = use_fit_end_date |> as.POSIXct(),
    lambda     = post_lambda,
...
pnbd_init_combined_fitsims_index_tbl |> glimpse()
```

We now use this setup to generate our simulations.


```{r generate_simulation_pnbd_init_combined_fitdata_transactions}
#| echo: TRUE
...
runsims_tbl <- pnbd_init_combined_fitsims_index_tbl |>
  anti_join(precomputed_tbl, by = "sim_file")


if(nrow(runsims_tbl) > 0) {
  pnbd_init_combined_fitsims_index_tbl <- runsims_tbl |>
    mutate(
      chunk_data = future_map2_int(
        cust_params, sim_file,
        run_simulations_chunk,
...
```{r retrieve_pnbd_init_combined_fit_simstats}
#| echo: TRUE

pnbd_init_combined_fit_simstats_tbl <- pnbd_init_combined_fitsims_index_tbl |>
  mutate(
    sim_data = map(
      sim_file, retrieve_sim_stats,
...
pnbd_init_combined_fit_simstats_tbl |> glimpse()
```


We now use this data to check how well our model fits the data. To do this we
construct combined transaction data.

```{r construct_simulation_transaction_data}
#| echo: true

pnbd_init_combined_fit_simtnx_tbl <- pnbd_init_combined_fit_simstats_tbl |>
  select(customer_id, draw_id, sim_data) |>
  unnest(sim_data)

pnbd_init_combined_fit_simtnx_tbl |> glimpse()
```

Finally, to create a comparison website we also want to remove the first
transaction from the fitted data so we can compare.

```{r remove_fitdata_initial_transaction}
#| echo: true

customer_comparison_tnxdata_tbl <- customer_fit_subset_tnxdata_tbl |>
  group_nest(customer_id) |>
  mutate(
    newdata = map(data, \(x) x |> tail(n = -1))
    ) |>
  select(customer_id, newdata) |>
  unnest(newdata)
  
customer_comparison_tnxdata_tbl |> glimpse()
```
...
```{r setup_simulation_pnbd_init_combined_validdata_transactions}
#| echo: TRUE
...
pnbd_init_combined_validsims_index_tbl <- pnbd_init_combined_simstats_tbl |>
  mutate(
    start_dttm = use_start,
    end_dttm   = use_final,
    lambda     = post_lambda,
...
pnbd_init_combined_validsims_index_tbl |> glimpse()
```
...
```{r generate_pnbd_init_combined_validdata_transactions}
#| echo: TRUE
...
runsims_tbl <- pnbd_init_combined_validsims_index_tbl |>
  anti_join(precomputed_tbl, by = "sim_file")


if(nrow(runsims_tbl) > 0) {
  pnbd_init_combined_validsims_index_tbl <- runsims_tbl |>
    mutate(
      chunk_data = future_map2_int(
        cust_params, sim_file,
        run_simulations_chunk,
...
```{r retrieve_pnbd_init_combined_valid_simstats}
#| echo: TRUE

pnbd_init_combined_valid_simstats_tbl <- pnbd_init_combined_validsims_index_tbl |>
  mutate(
    sim_data = map(
      sim_file, retrieve_sim_stats,
...
pnbd_init_combined_valid_simstats_tbl |> glimpse()
```
