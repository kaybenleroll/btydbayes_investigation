...
```{r fit_pnbd_init_short_twohiermean_stanmodel}
#| echo: TRUE

stan_modelname <- "pnbd_init_short_twohiermean"
...
if(!file_exists(stanfit_object_file)) {
  pnbd_init_short_twohiermean_stanfit <- pnbd_twohiermean_stanmodel$sample(
    data            =                stan_data_lst,
    chains          =                            4,
    iter_warmup     =                          500,
    iter_sampling   =                          500,
  ...
  pnbd_init_short_twohiermean_stanfit$save_object(stanfit_object_file, compress = "gzip")

} else {
  pnbd_init_short_twohiermean_stanfit <- read_rds(stanfit_object_file)
}

pnbd_init_short_twohiermean_stanfit$summary()
```


We have some basic HMC-based validity statistics we can check.

```{r calculate_pnbd_init_short_twohiermean_hmc_diagnostics}
#| echo: TRUE

pnbd_init_short_twohiermean_stanfit$cmdstan_diagnose()
```
...
```{r plot_pnbd_init_short_twohiermean_traceplots_nowarmup}
#| echo: TRUE
...
pnbd_init_short_twohiermean_stanfit$draws(inc_warmup = FALSE) |>
...
```{r plot_pnbd_init_short_twohiermean_parameter_neffratio}
#| echo: TRUE
...
```{r run_pnbd_init_short_twohiermean_assessment}
#| echo: TRUE

pnbd_stanfit <- pnbd_init_short_twohiermean_stanfit |>
  recover_types(customer_fit_stats_tbl)

pnbd_init_short_twohiermean_assess_data_lst <- run_model_assessment(
  model_stanfit    = pnbd_init_short_twohiermean_stanfit,
  insample_tbl     = customer_fit_stats_tbl,
  fit_label        = "pnbd_init_short_twohiermean",
...
pnbd_init_short_twohiermean_assess_data_lst |> glimpse()
```
...
```{r run_pnbd_init_short_twohiermean_fit_assessment}
#| echo: TRUE

simdata_tbl <- pnbd_init_short_twohiermean_assess_data_lst |>
...
```{r run_pnbd_init_short_twohiermean_valid_assessment}
#| echo: TRUE
...
# Fit Hierarchical Mean-Lambda, CoV-Lambda Model
...
```{r fit_pnbd_init_short_twohierlambda_stanmodel}
#| echo: TRUE

stan_modelname <- "pnbd_init_short_twohierlambda"
stanfit_prefix <- str_c("fit_", stan_modelname)
stanfit_seed   <- stanfit_seed + 1
...
if(!file_exists(stanfit_object_file)) {
  pnbd_init_short_twohierlambda_stanfit <- pnbd_twohierlambda_stanmodel$sample(
    data            =                stan_data_lst,
    chains          =                            4,
    iter_warmup     =                          500,
    iter_sampling   =                          500,
  ...
  pnbd_init_short_twohierlambda_stanfit$save_object(stanfit_object_file, compress = "gzip")

} else {
  pnbd_init_short_twohierlambda_stanfit <- read_rds(stanfit_object_file)
}

pnbd_init_short_twohierlambda_stanfit$summary()
```


We have some basic HMC-based validity statistics we can check.

```{r calculate_pnbd_init_short_twohierlambda_hmc_diagnostics}
#| echo: TRUE

pnbd_init_short_twohierlambda_stanfit$cmdstan_diagnose()
```
...
```{r plot_pnbd_init_short_twohierlambda_lambda_traceplots_nowarmup}
#| echo: TRUE
...
pnbd_init_short_twohierlambda_stanfit$draws(inc_warmup = FALSE) |>
  mcmc_trace(pars = parameter_subset) +
  expand_limits(y = 0) +
  labs(
    x = "Iteration",
...
```{r plot_pnbd_init_short_twohierlambda_parameter_neffratio}
#| echo: TRUE

pnbd_init_short_twohierlambda_stanfit |>
  neff_ratio(pars = c("lambda", "mu")) |>
  as.numeric() |>
  mcmc_neff() +
    ggtitle("Plot of Parameter Effective Sample Sizes")
...
```{r run_pnbd_init_short_twohierlambda_assessment}
#| echo: TRUE

pnbd_stanfit <- pnbd_init_short_twohierlambda_stanfit |>
  recover_types(customer_fit_stats_tbl)

pnbd_init_short_twohierlambda_assess_data_lst <- run_model_assessment(
  model_stanfit    = pnbd_init_short_twohierlambda_stanfit,
  insample_tbl     = customer_fit_stats_tbl,
  fit_label        = "pnbd_init_short_twohierlambda",
  fit_end_dttm     = use_fit_end_date     |> as.POSIXct(),
  valid_start_dttm = use_valid_start_date |> as.POSIXct(),
  valid_end_dttm   = use_valid_end_date   |> as.POSIXct(),
  sim_seed         = 4220
...
pnbd_init_short_twohierlambda_assess_data_lst |> glimpse()
```
...
```{r run_pnbd_init_short_twohierlambda_fit_assessment}
#| echo: TRUE

simdata_tbl <- pnbd_init_short_twohierlambda_assess_data_lst |>
  use_series(model_fit_simstats_filepath) |>
  read_rds()
...
```{r run_pnbd_init_short_twohierlambda_valid_assessment}
#| echo: TRUE
...
model_assess_tbl <- dir_ls("data", regexp = "pnbd_init_short_(fixed|one|two).*_assess_.*simstats") |>
...
model_assess_tbl |> write_rds("data/assess_data_pnbd_init_short_twohier_tbl.rds")
```
